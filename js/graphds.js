...
$(function() {
  ...
  $('#tutorial-8 .tutorial-next').click(function() {
    window.scrollTo(0,0);
  });
});
...
function GraphVisu(arg1, arg2, arg3)
{
  ...
  for (var i = 0; i < links.length; i++)
  {
    ...
    obj.weight = 0;
    if (UNWEIGHTED === false) obj.weight = links[i].weight;
    
    obj.state = "default";
    obj.displayWeight = false;
    
    if (UNWEIGHTED === false) obj.displayWeight = true;
  ...
  console.log(json);
    ...
    if (IsTree === true) d3.select("#isTree").text(" : 是");
    else d3.select("#isTree").text(" : 否");
    ...
    if (IsComplete === true) d3.select("#isComplete").text(" : 是");
    else d3.select("#isComplete").text(" : 否");
    ...
    if (IsBipartite === true) d3.select("#isBipartite").text(" : 是");
    else d3.select("#isBipartite").text(" : 否");
    
    // IsDAG
    
    var IsDAG = true;
    if (UNDIRECTED === true) IsDAG = false;
    else
    {
    ...
    function dfs(u,v)
    {
      ...
      for (var k = 0; k < AdjList[v].length; k++)
        if (visited[AdjList[v][k]] === false) dfs(v,AdjList[v][k]);
    }
    }
    
    if (IsDAG === true) d3.select("#isDAG").text(" : 是");
    else d3.select("#isDAG").text(" : 否");
  ...
  function keydown() {
    ...
    case 13: //enter
      if (selected_link && UNWEIGHTED === false)
      {
        while (true)
      {
        var newWeight = prompt("键入新的长度 : ( <= 99)");
